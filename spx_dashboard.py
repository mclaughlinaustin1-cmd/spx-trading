...
import ta  # Technical Analysis library for RSI/MACD (pip install ta-lib if needed)
import logging
...
with st.sidebar:
    ...
    api_key = st.text_input("Polygon API Key", type="password")  # Secure input
...
def fetch_data():
    if not client:
        st.error("Polygon API key required for live data.")
        return None
    end = datetime.now()
    start = end - timedelta(days=5)  # More history for better indicators/ML
    bars = client.get_aggs("SPY", 15, "minute", start.strftime("%Y-%m-%d"), end.strftime("%Y-%m-%d"))
    if not bars:
        return None
