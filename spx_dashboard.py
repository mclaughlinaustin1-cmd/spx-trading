...
while True:
    with placeholder.container():
        ...
        # Rename 'Adj Close' to 'Close' if Close missing
        if "Close" not in spx.columns and "Adj Close" in spx.columns:
            spx.rename(columns={"Adj Close": "Close"}, inplace=True)

        # Keep required columns
        required_cols = ["Open", "High", "Low", "Close", "Volume"]
        missing = [col for col in required_cols if col not in spx.columns]
        if missing:
            st.warning(f"Missing columns: {missing}. Retrying...")
            time.sleep(refresh_interval)
            continue
